<!DOCTYPE html>
<html lang="en" dir="ltr">
<head>
    <meta charset="utf-8">
    <title></title>
    <link rel="stylesheet" href="assets/css/style.css">
</head>
<body>
<header id="header">
    <div class="left">
        <div class="flexbox vc sb">
            <div class="col">
                <a class="logo-wrapper" href="/">
                    <img class="logo lazy" data-src="" alt="">
                </a>
            </div>
            <div class="col">
                <button class="toggle-sidebar">
                    <span class="line"></span>
                    <span class="line"></span>
                    <span class="line"></span>
                </button>
            </div>
        </div>
    </div>
    <div class="right">
        <div class="container">
            <div class="flexbox vc">
                <div class="col">
                    <button class="icon-wrapper search-toggle">
                        <img class="icon lazy" data-src="/global/assets/img/search.svg" alt="">
                        <p>Search</p>
                    </button>
                </div>
                <div class="col m-left">
                    <button class="icon-wrapper user-add">
                        <img class="icon lazy" data-src="/global/assets/img/user-add.svg" alt="Users">
                    </button>
                </div>
                <div class="col">
                    <button class="icon-wrapper notifications">
                        <img class="icon lazy" data-src="/global/assets/img/notifications.svg" alt="Notifications">
                    </button>
                </div>
                <div class="col">
                    <button class="icon-wrapper question">
                        <img class="icon lazy" data-src="/global/assets/img/question-mark.svg" alt="Questions">
                    </button>
                </div>
                <div class="col">
                    <div class="user-wrapper">
                        <button class="icon-wrapper user">
                            <img class="icon lazy" data-src="/global/assets/img/icon-placeholder-round.svg" alt="Placeholder">
                            <p>Mollie Jennings</p>
                        </button>
                    </div>
                </div>
            </div>
        </div>
    </div>
</header>
<div class="dashboar-wrapper">
